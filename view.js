...
db.serialize(function(){
  ...
  // gender FROM Votes LEFT JOIN Politicians ON Votes.politicianId = Politicians.id
  // LEFT JOIN Voters ON Votes.voterId = Voters.id ORDER BY totalVote DESC`, function(err,rows){
  //   if(err){
  //     console.log(err);
  //   } else {
  //     console.log(rows);
  ...
  db.all(`SELECT count(*) AS totalVote, firstName||' '||lastName AS name, gender, age FROM Votes
  LEFT JOIN Voters ON Voters.id = Votes.voterId GROUP BY name HAVING totalVote > 1
